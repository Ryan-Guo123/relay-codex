...
The next gate is Define: turn the research into one primary use case, one demo path, and one public message. The current Define gate is tracked in [define-gate.md](define-gate.md), the validation plan is tracked in [validation-plan.md](validation-plan.md), the stricter competitor map is tracked in [market-map.md](market-map.md), community pain research is tracked in [community-research.md](community-research.md), and the stricter evidence and business case is tracked in [evidence-and-business-case.md](evidence-and-business-case.md).
